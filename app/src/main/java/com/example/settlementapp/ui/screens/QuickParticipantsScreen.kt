...
fun QuickParticipantsScreen(
    ...
    val participants = quickSession.participants
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(s.quickParticipantsTitle) },
                navigationIcon = {
        ...
        LazyColumn(
                ...
                AppCard {
                    ...
                    Button(
                        onClick = {
                            ...
                            viewModel.registerQuickParticipantsFromCounts(
                                maleCount = maleN,
                                femaleCount = femaleN,
            ...
            items(participants, key = { it.id }) { p ->
                ...
                ParticipantEditRow(
                    ...
                    participant = p,
                    onGenderChange = { viewModel.updateQuickParticipant(p.copy(gender = it)) },
                    onPaymentChange = { viewModel.updateQuickParticipant(p.copy(paymentType = it)) },
                    onSave = {
                        val newName = editedNames[p.id]?.trim().orEmpty()
                        if (newName.isNotBlank() && newName != p.name) {
                            viewModel.updateQuickParticipant(p.copy(name = newName))
                        }
                        editedNames.remove(p.id)
                    },
                    onDelete = {
                        editedNames.remove(p.id)
                        viewModel.deleteQuickParticipant(p)
                    }
                )
