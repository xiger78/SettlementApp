#!/usr/bin/env python3
"""Capture Chinese home screen for MANUAL_zh.md."""
import os
import re
import subprocess
import sys
...
def is_home():
    ...
    if "结算金额" in t:
        return False
    if ZH["home_marker"] in t and ZH["menu"] in t:
        return True
    if "最近聚会" in t and "设置" in t:
        return True
    return False
...
def set_chinese():
    ...
    sleep(18)
    open_settings()
    if not wait_text("中文", timeout=5) and "Language" not in all_text() and "语言" not in all_text():
    ...
    sleep(1.2)
    force_home()
    if ZH["app_name"] not in all_text():
        raise RuntimeError(f"Chinese not applied: {all_text()[:300]!r}")


def shot():
    sh("shell", "input", "keyevent", "111")
    sleep(0.4)
    path = os.path.join(SHOT_DIR, "01_home_zh.png")
    sh("shell", "screencap", "-p", "/sdcard/s.png")
    sh("pull", "/sdcard/s.png", path)
    size = os.path.getsize(path)
    print(f"SHOT 01_home_zh.png ({size:,} bytes)")
    if size > 400_000:
        raise RuntimeError("Screenshot too large (likely launcher)")
    if ZH["app_name"] not in all_text() and ZH["home_marker"] not in all_text():
        raise RuntimeError("Screenshot taken but Chinese home markers missing")
...
def main():
    ...
    set_chinese()
    force_home()
    if ZH["menu_meeting"] not in all_text():
        raise RuntimeError(f"Not on Chinese home: {all_text()[:300]!r}")
    shot()
    print("Done.")
